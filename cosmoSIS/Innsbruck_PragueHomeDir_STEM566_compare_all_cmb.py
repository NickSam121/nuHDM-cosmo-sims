...
nuHDM           = np.genfromtxt(fname="/media/nsamaras/T7/Archive/cosmosis-standard-library_prague_home_directory/output/angus/cmb_cl/tt.txt", delimiter="")
camb_nuHDM  = np.genfromtxt(fname="/home/nsamaras/Desktop/astro/AUUK_CAMB+CLASS/CAMB-1.3.5/fortran/angus_nils_scalCls.dat", delimiter="")
camb_opt_nuHDM  = np.genfromtxt(fname="/home/nsamaras/Desktop/astro/AUUK_CAMB+CLASS/CAMB-1.3.5/fortran/angus_opt_scalCls.dat", delimiter="")
opt_nuHDM    = np.genfromtxt(fname="/media/nsamaras/T7/Archive/cosmosis-standard-library_prague_home_directory/output/opt-angus/cmb_cl/tt.txt", delimiter="")
planck            = np.genfromtxt(fname="/media/nsamaras/T7/Archive/cosmosis-standard-library_prague_home_directory/NS_data/COM_PowerSpect_CMB-TT-binned_R3.01.txt", delimiter="")
...
plt.plot(camb_opt_nuHDM1, camb_opt_nuHDM2, label = r"CAMB opt-$\nu$HDM (never in paper)", color = "teal", linestyle = "--", linewidth= 1.5)
plt.plot(opt_nuHDM, label = r"CosmoSIS opt-$\nu$HDM Samaras+ 25", color = "green", linestyle = "-", linewidth= 3.0)
plt.plot(nuHDM, label = r"CosmoSIS $\nu$HDM (never in paper)", color = "red", linestyle = "-", linewidth= 3.0)
