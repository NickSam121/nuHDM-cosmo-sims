...
subroutine gravana(x,force,dx,ncell)
  ...
  if (gravity_type==0) then
     ...
     if (connected_Mond) then
        if (Activate_g_ext) then !Added by IB based on Arxiv: 1509.08457.
            ...
            if (cosmo) then
                do i = 1, ncell
                    ...
                    force(i, :) = constant*r_inv*r_inv*(K_0_ext*cos_theta*g_ext_dir(:) + r_inv*x(i, :)*(1.0d0 + 0.5d0*K_0_ext*(1.0d0 - 3.0d0*cos_theta*cos_theta)))
                end do
            else
                do i = 1, ncell
                    r_inv = 1/r(i)
        ...
        ! Compute MONDian acceleration a_i = -sqrt(G*M*a0*aexp)*x_i/r^2  (a ~ 1/r)
        constant = -sqrt(multipole(1)*a0*aexp**4) !aexp factor added by IB for cosmo runs.
        !do idim=1,ndim
        !   do i=1,ncell
...
subroutine phi_ana(xx, rr, pp, ngrid)
  ...
  real(dp)::constant
    ...
    if (cosmo) then !Block added by IB for cosmo runs.
        fourpi = 1.5D0*omega_m*aexp
    else
        fourpi  = 4.0d0*PI*1.0d0
    end if

#if NDIM==1
  fourpi = 4.D0*ACOS(-1.0D0)
  constant = multipole(1)*fourpi/2.0d0
  do i = 1,ngrid
     pp(i) = constant*rr(i)
  end do
#endif
#if NDIM==2
  constant = multipole(1)*2.0d0
  do i = 1,ngrid
...
if (Activate_g_ext) then !Added by IB based on Arxiv: 1509.08457.
    ...
    if (cosmo) then
        do i = 1,ngrid
            ...
            pp(i) = constant*r_inv*(1.0d0 + 0.5d0*K_0_ext*(1 - cos_theta*cos_theta))
        end do
    else
        do i = 1,ngrid
            r_inv = 1/rr(i)
