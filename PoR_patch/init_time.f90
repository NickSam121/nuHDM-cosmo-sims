...
!Major alteration IB: omega_m changed to omega_m_true in many places.
!omega_m_true added in output_amr and init_amr so it is available for a restarted simulation.
  ...
  if(cosmo)then
     ...
     call friedman(dble(omega_m_true),dble(omega_l),dble(omega_k), &
          & 1.d-6,dble(aexp_ini), &
          & aexp_frw,hexp_frw,tau_frw,t_frw,n_frw)
!******************look up table output NW*************************************************
!    do i=0, n_frw
!     open(1337, file='Friedman_table.dat')
!     write(1337, '(4e20.6)') aexp_frw(i), hexp_frw(i), tau_frw(i), t_frw(i)
!    end do
!******************************************************************************************
  ...
  if(cooling.and..not.(neq_chem.or.rt))then
     ...
     if(cosmo)then
        ...
        call set_model(Nmodel,dble(J21*1d-21),-1.0d0,dble(a_spec),-1.0d0,dble(z_reion), &
             & -1,2, &
             & dble(h0/100.0d0),dble(omega_b),dble(omega_m_true),dble(omega_l), &
  ...
  if(neq_chem.or.rt) then
     ...
     if(cosmo)then
        ...
        call rt_set_model(Nmodel,dble(J21*1d-21),-1.0d0,dble(a_spec),-1.0d0,dble(z_reion), &
             & -1,2, &
             & dble(h0/100.),dble(omega_b),dble(omega_m_true),dble(omega_l), &
             & dble(aexp_ini),T2_sim)
        T2_start=T2_sim
        if(nrestart==0)then
           if(myid==1)write(*,*)'Starting with T/mu (K) = ',T2_start
...
subroutine init_file
  ...
  real(sp)::dxini0,xoff10,xoff20,xoff30,astart0,omega_m0,omega_b0,omega_l0,h00
!***************************added omega_b0 NW********************************************
  ...
  do ilevel=levelmin,nlevelmax
     if(initfile(ilevel).ne.' ')then
        ...
        read(10)n1(ilevel),n2(ilevel),n3(ilevel),dxini0 &
             ...
             & ,astart0,omega_m0,omega_b0,omega_l0,h00
!*****************************added omega_b0 NW******************************
        close(10)
...
!*********************added omega_b0 NW*********************************
           ...
           !if(hydro)omega_b= 0.04931 !Moved from later and updated by IB from 0.045 based on https://arxiv.org/abs/1807.06209, 0.0224/(0.674*0.674).
           !!!if(hydro)omega_b=0.999999*omega_m
           if(hydro)omega_b = omega_b0
!*****************changed hard wired value of omega_b to reading it in from graphic2 file NW****************************

!Major alteration IB, original: omega_m=omega_m0.
            omega_m_true = omega_m0
            if (Uniform_DM) then
                omega_m = omega_b
            else
...
!Major alteration IB, newly added.
!Reduced mass_sph above will improve the resolution, so it is acceptable.
  ...
  omega_k=1.d0-omega_l-omega_m_true
  ...
  do ilevel=levelmin,nlevelmax_part
     ...
     vfact(ilevel)=astart(ilevel)*fpeebl(astart(ilevel)) & ! Same scale factor as in grafic1
          & *sqrt(omega_m_true/astart(ilevel)+omega_l*astart(ilevel)*astart(ilevel)+omega_k) &
          & /astart(ilevel)*h0
  end do
  ...
  vfact(1)=aexp*fpeebl(aexp)*sqrt(omega_m_true/aexp+omega_l*aexp*aexp+omega_k)
...
contains
  ...
  function fy(a)
    ...
    y=omega_m_true*(1.d0/a-1.d0) + omega_l*(a*a-1.d0) + 1.d0
    fy=1.d0/y**1.5d0
...
!!$  end function ad1
  ...
  function fpeebl(a)
    ...
    fpeebl=(omega_l*a*a-0.5d0*omega_m_true/a)/y - 1.d0 + a*fy(a)/fact
    return
  end function fpeebl
  !cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
