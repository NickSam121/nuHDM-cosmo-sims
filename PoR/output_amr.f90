...
subroutine dump_all
  ...
  if(ndim>1)then
     ...
     if (myid==1) print *,'Safety sleep for ',safety_secs,' secs...'
...
subroutine backup_amr(filename)
  ...
  write(ilun)omega_m,omega_m_true,omega_l,omega_k,omega_b,h0,aexp_ini,boxlen_ini !omega_m_true added by IB.
