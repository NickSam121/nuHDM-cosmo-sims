...
This repository describes how to conduct cosmological MOND structure-formation simulations with Phantom of RAMSES (PoR - which is a MONDified version of RAMSES), on the $\nu$ HDM framework (with/without Star-Formation/External Field Effect). Owing its origin to [Angus et al. 2011](https://ui.adsabs.harvard.edu/abs/10.1111/j.1365-2966.2011.19321.x), and to [Katz et al. 2013](https://ui.adsabs.harvard.edu/abs/2013ApJ...772...10K/abstract), this model replaces Newtonian gravity with Milgrom's [QUMOND](https://ui.adsabs.harvard.edu/abs/2010MNRAS.403..886M/abstract) and the Cold Dark Matter particle with a light, neutral, right-handed, sterile neutrino of rest mass $m_{\nu_s} \approx$ 11 eV. This is a rather conservative idea, meaning that the Big Bang initial singularity, the cosmological inflation and the FLRW metric (background history) are all kept in place. 
...
The CAMB opt-nuHDM will NOT fit the Planck CMB. While the opt-nuHDM was a nearly perfect fit to the CMB, with CosmoSIS mcmc sampler, if one writes the output posterior parameters into CAMB, one simple does not find any correspondance and the fit is bad. I solved this problem, by scaling the resulting z = 199 transfer function, so that the CAMB transfer function is the same as the CosmoSIS one. What I eventually need is the transfer function at z = 199.0. It does not matter if the cmb does not match the Planck data. This is not the best way to do it, but one needs to understand the different physics that the two codes are making use of. In the end, one needs not the CMB but the Transfer function for MUSIC. Obviously, one obtains one transfer function for nuHDM and one for the opt-nuHDM, running the corresponding namelists. Once they are generated, each of them individually go to MUSIC. For more details, see Samaras & Kroupa 2026, subm.
